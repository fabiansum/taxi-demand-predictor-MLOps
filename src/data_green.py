...
def download_one_file_of_green_raw_data(year: int, month: int) -> Path:
    """
    ...
    if response.status_code == 200:
        ...
        year_month_dir = RAW_DATA_DIR / f'year={year}/month={month:02d}'
        year_month_dir.mkdir(parents=True, exist_ok=True)

        # create file path
        path = year_month_dir / f'green_tripdata_{year}-{month:02d}.parquet'
...
def load_green_raw_data(
    ...
    if months is None:
        ...
        # download data for a single month
        months = [months]

    for month in months:

        local_file = RAW_DATA_DIR / f'year={year}/month={month:02d}/green_tripdata_{year}-{month:02d}.parquet'
        if not local_file.exists():
            try:
                # download the file from the NYC website
                print(f'Downloading file {year}-{month:02d}')
                download_one_file_of_green_raw_data(year, month)
            except:
...
def download_green_raw_data_from_2013_to_present_year() -> pd.DataFrame:
    """
    Downloads Parquet file with historical taxi rides data from August 2013 to present year
    """
    rides = pd.DataFrame()
    ...
    for year in range(2013, current_year+1):
        ...
        if year == 2013:
            # data only available from August 2013
            months = list(range(8, 13))

        if year == current_year:
            # only download data for current month
